    ...
    async def _monitor(self, page: Page) -> None:
        ...
        while True:
            try:
                ...
                context = full_text[-self.settings.context_characters :]
...
For comparisons about fluency, manners, skills, or behavior, make claims only when the visible conversation provides direct support. Name the observable examples briefly. If the context does not provide enough evidence, say that clearly instead of inventing a ranking, member fact, or history. Do not generate image, voice, song, or edit requests in normal chat—tell users to use the explicit slash command if relevant. Output only the message that should be posted."""
        try:
